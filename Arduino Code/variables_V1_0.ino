...
// COdigos de comando
#define CABECERA      0xFF
#define OP_ESCRIBIR   0x01
#define OP_LEER       0x00
...
void setup() {
  ...
  pinMode(pin_dig_in_D2,INPUT);
  pinMode(pin_dig_in_D3,INPUT);
...
//Version Binaria
uint8_t entero_de(byte hi,byte lo) {
uint8_t value;
value=(((uint8_t)hi)<<8)|lo;
return value;
}

byte v_low(uint8_t a) {
  return (byte)(a&0xFF);
}
byte v_high(uint8_t a) {
  return (byte)(a>>8);
}
...
int Ejecuta_Comando() {
 ...
 // primer byte 'F'
  if (buffer_com[0]!=CABECERA) {
    //Serial.print("Inicio no valido, esperado F\n");
  } else {
    // Identificacion de variable
    switch (buffer_com[1]) {
      case ID_DIG_OUT_S0:
            if (buffer_com[2]==OP_ESCRIBIR) {v_S0=buffer_com[4];digitalWrite(pin_dig_out_S0,v_S0);}
            else Envia_Respuesta(ID_DIG_OUT_S0,v_S0);
      break;
      case ID_DIG_OUT_S1:
            if (buffer_com[2]==OP_ESCRIBIR) {v_S1=buffer_com[4];digitalWrite(pin_dig_out_S1,v_S1);}
            else Envia_Respuesta(ID_DIG_OUT_S1,v_S1);
      break;
      case ID_DIG_OUT_S2:
            if (buffer_com[2]==OP_ESCRIBIR) {v_S2=buffer_com[4];digitalWrite(pin_dig_out_S2,v_S2);}
            else Envia_Respuesta(ID_DIG_OUT_S2,v_S2);
      break;
      case ID_DIG_OUT_S3:
            if (buffer_com[2]==OP_ESCRIBIR) {v_S3=buffer_com[4];digitalWrite(pin_dig_out_S3,v_S3);}
